...
// GET /api/artists/video-thumbnail?artistId=...&videoId=...
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (artist.plan !== "pro") return NextResponse.json({ error: "Custom video thumbnails require a Pro plan." }, { status: 403 })

    const timestamp = Date.now()
    const filePath = `artists/${artist.id}/videos/${videoId}/${timestamp}.webp`
